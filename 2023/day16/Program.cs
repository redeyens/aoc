...
var possibleStartRays = Enumerable.Range(0, input.Count).Select(row => (startRow: 0, startCol: int.MinValue, endRow: 0, endCol: int.MaxValue))
    .Concat(Enumerable.Range(0, input.Count).Select(row => (startRow: 0, startCol: int.MaxValue, endRow: 0, endCol: int.MinValue)))
    .Concat(Enumerable.Range(0, input[0].Length).Select(col => (startRow: int.MinValue, startCol: col, endRow: int.MaxValue, endCol: col)))
    .Concat(Enumerable.Range(0, input[0].Length).Select(col => (startRow: int.MaxValue, startCol: col, endRow: int.MinValue, endCol: col)));

int energy = possibleStartRays.Select(ray => GetEnergy(elements, ray, input.Count - 1, input[0].Length - 1)).Max();
...
static int GetEnergy(List<(char c, int row, int col)> elements, (int startRow, int startCol, int endRow, int endCol) ray, int maxRow, int maxCol)
{
    ...
    while (rays.Count > 0)
    {
        ...
        for (int i = Math.Max(0, Math.Min(cR.startRow, cR.endRow)); i <= Math.Min(Math.Max(cR.startRow, cR.endRow), maxRow); i++)
        {
            for (int j = Math.Max(0, Math.Min(cR.startCol, cR.endCol)); j <= Math.Min(Math.Max(cR.startCol, cR.endCol), maxCol); j++)
            {
                energized.Add((i, j));
            }
        }
