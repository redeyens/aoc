...
namespace day19
{
    class Program
    {
        ...
        {
            var input = finalInput;

            var rulesDef = GetLines(input)
                .TakeWhile(l => !string.IsNullOrEmpty(l));
            
            var text = GetLines(input)
                .SkipWhile(l => !string.IsNullOrEmpty(l))
                .Skip(1);

            var ruleRepo = new RulesRepository(rulesDef);

            Console.WriteLine(ruleRepo);
            Console.WriteLine("Done! Rules matching {0}.", text.Where(line => ruleRepo.MasterRule.IsExactMatch(line)).Count());
        }
	    ...
	    private static string testInput = @"42: 9 14 | 10 1
9: 14 27 | 1 26
10: 23 14 | 28 1
1: ""a""
11: 42 31 | 42 11 31
...
aabbbbbaabbbaaaaaabbbbbababaaaaabbaaabba";
 	    ...
 	    private static string finalInput = @"66: 69 116 | 9 115
8: 42 | 42 8
11: 42 31 | 42 11 31
91: 95 9 | 109 69
14: 110 69 | 15 9
