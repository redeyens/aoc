...
public class Program
{
	public static void Main()
	{
		...
		{
			string[] policyPassPair = line.Split(':');
			string policy = policyPassPair[0];
			string pass = policyPassPair[1];
			
			string[] policyParts = policy.Split(' ');
			string occurences = policyParts[0];
			string letter = policyParts[1];
			
			string[] occurencesParts = occurences.Split('-');
			int minOccurences = Int32.Parse(occurencesParts[0]);
			int maxOccurences = Int32.Parse(occurencesParts[1]);
			
			Console.WriteLine("{0}-{1} {2}: {3}", minOccurences, maxOccurences, letter, pass);
			
			int letterCount = pass.Length - pass.Replace(letter, "").Length;
			
			int matchingLetters = 0;
			if(pass[minOccurences] == letter[0])
			{
				matchingLetters++;
			}
			
			if(pass[maxOccurences] == letter[0])
			{
				matchingLetters++;
			}
		...
		Console.WriteLine("{0}", validPassCount);
	}

	public static string input = @"9-12 q: qqqxhnhdmqqqqjz
