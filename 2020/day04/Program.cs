...
public class Program
{
	public static void Main()
	{
		...
		Console.WriteLine("{0} valid passports", validPassports);
	}
	...
	private static ValidationRule[] rules = new ValidationRule[] {
		new ValidationRule ("byr", new Regex(@"byr:(\d{4})", RegexOptions.Compiled | RegexOptions.IgnoreCase), (new IntRangeCheck(1920, 2002).IsOK)),
		new ValidationRule ("iyr", new Regex(@"iyr:(\d{4})", RegexOptions.Compiled | RegexOptions.IgnoreCase), (new IntRangeCheck(2010, 2020).IsOK)),
		new ValidationRule ("eyr", new Regex(@"eyr:(\d{4})", RegexOptions.Compiled | RegexOptions.IgnoreCase), (new IntRangeCheck(2020, 2030).IsOK)),
		new ValidationRule ("hgt", new Regex(@"hgt:(\d{3}cm|\d{2}in)", RegexOptions.Compiled | RegexOptions.IgnoreCase), IsValidHgt),
		new ValidationRule ("hcl", new Regex(@"hcl:#[0-9a-f]{6}", RegexOptions.Compiled | RegexOptions.IgnoreCase), IsValidAny),
		new ValidationRule ("ecl", new Regex(@"ecl:(amb|blu|brn|gry|grn|hzl|oth)", RegexOptions.Compiled | RegexOptions.IgnoreCase), IsValidAny),
		new ValidationRule ("pid", new Regex(@"pid:\d{9}(\s|$)", RegexOptions.Compiled | RegexOptions.IgnoreCase), IsValidAny)};
	
	private static bool IsValidAny(string input)
	{
		return true;
	}
	
	private static bool IsValidHgt(string input)
	{
		IntRangeCheck check;
		
		if(input.EndsWith("cm"))
			check = new IntRangeCheck(150, 193);
		else
			check = new IntRangeCheck(59, 76);
		
		return check.IsOK(input.TrimEnd('c', 'm', 'i', 'n'));
	}
	
	private static string input = @"byr:2024 iyr:2016
